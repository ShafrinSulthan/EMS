...
const PrintPayslip = () => {
  ...
  if(!payslip) return <p className="text-center py-2 text-slate-400">Payslip not found</p>
  ...
  return (
    <div className="max-w-2xl mx-auto p-8 bg-white animate-fade-in">
      <div className="text-center border-b border-slate-200 pb-6 mb-8">
        ...
        <p className="text-slate-500 text-sm mt-1">{format(new Date(payslip.year, payslip.month - 1), "MMM yyy")}</p>
      </div>
      <div className="grid grid-cols-2 gap-6 mb-8">
        ...
        <div>
          ...
          <p className="font-semibold text-slate-900">{format(new Date(payslip.year, payslip.month - 1), "MMM yyy")}</p>
        </div>
