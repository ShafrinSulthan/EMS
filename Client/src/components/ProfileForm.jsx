...
const ProfileForm = ({initialData, onSuccess}) => {
  ...
  return (
    <form onSubmit={handleSumbit} className='card p-5 sm:p-6 mb-6'>
        ...
        {error && (
            <div className='bg-rose-50 text-rose-700 p-4 rounded-xl text-sm border border-rose-200 mb-6 flex items-start gap-3'>
                <div className='w-1.5 h-1.5 rounded-ful bg-rose-500 mt-1.5 shrik-0'/>
                {error}
            </div>
        )}
        {message && (
            <div className='bg-emerald-50 text-emerald-700 p-4 rounded-xl text-sm border border-emerald-200 mb-6 flex items-start gap-3'>
                <div className='w-1.5 h-1.5 rounded-ful bg-emerald-500 mt-1.5 shrik-0'/>
                {message}
            </div>
        )}
            ...
            {initialData.isDeleted ? (
                <div className='pt-2'>
                    <div className='p-4 bg-rose-50 border border-rose-200 rounded-xl text-center'>
                        <p className='text-rose-600 font-medium tracking-tight'>Account Deactive</p>
                        <p className='teext-sm text-rose-500 mt-0.5'>You can no longer update your profile.</p>
                    </div>
                </div>
            ) : (
                <div className='flex justify-end pt-2'>
                    ...
                    className='btn-primary flex items-center gap-2 justify-center w-full sm:w-auto'>
                        {loading ? <Loader2 className='w-5 h-5 animate-spain'/> : <Save className='w-5 h-5'/>}
                        Save Changes
                    </button>
                </div>
            )}
